...
module Functions (F : Ctypes.FOREIGN) = struct
  ...
  let version_str = foreign "jansson_version_str" (void @-> returning string)

  module Json = struct
    let equal =
      foreign "json_equal" (ptr T.Json.t @-> ptr T.Json.t @-> returning bool)
    ...
    let to_string =
      foreign "json_string_value" (ptr T.Json.t @-> returning Ctypes.string)
    ...
    module Number = struct
      ...
      let to_int =
        foreign "json_integer_value" (ptr T.Json.t @-> returning llong)

      let float = foreign "json_real" (double @-> returning @@ ptr T.Json.t)

      let to_number =
        foreign "json_number_value" (ptr T.Json.t @-> returning double)
    end
    ...
    module Array = struct
      ...
      let append_new =
        foreign "json_array_append_new"
          (ptr T.Json.t @-> ptr T.Json.t @-> returning int)

      let size = foreign "json_array_size" (ptr T.Json.t @-> returning int)

      let get =
        foreign "json_array_get"
          (ptr T.Json.t @-> int @-> returning @@ ptr T.Json.t)
    end
    ...
    module Object = struct
      ...
      let set_new =
        foreign "json_object_set"
          (ptr T.Json.t @-> Ctypes.string @-> ptr T.Json.t @-> returning int)

      module Iter = struct
        let create =
          foreign "json_object_iter" (ptr T.Json.t @-> returning (ptr void))

        let key =
          foreign "json_object_iter_key" (ptr void @-> returning Ctypes.string)

        let value =
          foreign "json_object_iter_value"
            (ptr void @-> returning (ptr T.Json.t))

        let next =
          foreign "json_object_iter_next"
            (ptr T.Json.t @-> ptr void @-> returning (ptr void))
      end
    end
  end
end
